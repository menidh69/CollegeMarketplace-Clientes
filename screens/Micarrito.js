...
import { StyleSheet, TabBarIOS, Text, View, Image, TouchableOpacity, TouchableHighlight, ActivityIndicator, FlatList, Modal, Alert } from 'react-native';
    ...
    const crearOrden = async () => {
        ...
        if (pago == "cash") {
            ...
            if (deviceSession) {
                setModalText("Creando pedido")
                setLoading(true)
                const crear = await fetch('http://college-mp-env.eba-kwusjvvc.us-east-2.elasticbeanstalk.com/api/v2/openpay/create_charge',
                    {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({ "user_id": user.id, "device_session_id": deviceSession })
                    })
                const resp = await crear.json();
                console.log(resp)
                if (resp.error) {
            ...
            return (
                <View style={styles.container2} >
                    ...
                    <View style={{ marginBottom: 20, marginLeft: 20 }}>
                        ...
                        <Text>Total: ${Number.parseFloat(getTotaPrecio()).toFixed(2)} </Text>
                    </View>
                        ...
                        <View style={styles.centeredView}>
                            <View style={styles.modalView}>
                                ...
                                <DropDownPicker
                                    ...
                                    defaultValue={pago}
                                    containerStyle={{ height: 40, width: '40%', marginBottom: 10, marginTop: 10 }}
...
const Producto = ({ producto }) => {
    ...
    const eliminar = async (id) => {
        console.log('hola')
        const eliminar = await fetch(`http://college-mp-env.eba-kwusjvvc.us-east-2.elasticbeanstalk.com/api/v1/carrito/items/${id}`,
            {
                method: "DELETE",
                headers: { "Content-Type": "application/json" }
            })
